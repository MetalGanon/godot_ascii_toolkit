...
func _ready():
	assert(
		...
		"Grid of ASCIILabel named %s is not conforming..." % name
	)
...
func _size_tile_changed() -> bool:
	...
	if (size_tile.x != int(size.x/GodotASCIIToolKit.TILE_SIZE_PX.X) or 
		size_tile.y != int(size.y/GodotASCIIToolKit.TILE_SIZE_PX.Y)
	):
		return true
	else:
		return false
...
func _update_size_tile() -> void:
	...
	size_tile.x = int(size.x / GodotASCIIToolKit.TILE_SIZE_PX.X)
	size_tile.y = int(size.y / GodotASCIIToolKit.TILE_SIZE_PX.Y)
...
func _position_tile_changed() -> bool:
	...
	var new_p_tile_x = int(position_tile.x/GodotASCIIToolKit.TILE_SIZE_PX.X) 
	var new_p_tile_y = int(position_tile.y/GodotASCIIToolKit.TILE_SIZE_PX.Y)
...
func _update_position_tile() -> void:
	## Compute the size of the current control node in tiles
	position_tile.x = int(position.x / GodotASCIIToolKit.TILE_SIZE_PX.X)
	position_tile.y = int(position.y / GodotASCIIToolKit.TILE_SIZE_PX.Y)
...
func check_grid_conforming() -> bool:
	...
	if (int(size.x) % GodotASCIIToolKit.TILE_SIZE_PX.X != 0 or 
		int(size.y) % GodotASCIIToolKit.TILE_SIZE_PX.Y != 0 or
		int(position.x) % GodotASCIIToolKit.TILE_SIZE_PX.X != 0 or
		int(position.y) % GodotASCIIToolKit.TILE_SIZE_PX.X != 0
	):
		return false
	return true
